...
def pushSHCBundle(shcURI, sessionKey, target):
    r = requests.post("https://{}:8089/services/apps/deploy".format(shcURI),
                      data="target={}&action=all&advertising=true".format(target),
...
def createServerClass(deploymentServerURI, serverClass, sessionKey):
    r = requests.post("https://{}:8089/services/deployment/server/serverclasses".format(deploymentServerURI), data=serverClass,
                      headers={'Authorization': ('Splunk %s' % sessionKey)},
                      verify=False)
    logger.info(r.text)


def archiveApp(searchHeadURI, appName, sessionKey):
    r = requests.post('https://{}:8089/services/apps/local/{}/package'.format(searchHeadURI, appName),
                      headers={'Authorization': ('Splunk %s' % sessionKey)},
                      verify=False)
    logger.info(r.text)
